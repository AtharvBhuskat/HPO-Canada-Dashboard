...
const icons: Record<Platform, { label: string; color: string; svg: string }> = {
  linkedin: {
    label: 'LinkedIn',
    color: '#0A66C2',
...
export default function PlatformIcon({ platform, size = 20, showLabel = false }: Props) {
  const { label, color, svg } = icons[platform]
  return (
    <span className="inline-flex items-center gap-1.5">
      <svg width={size} height={size} viewBox="0 0 24 24" fill={color} aria-label={label}>
        <path d={svg} />
      </svg>
